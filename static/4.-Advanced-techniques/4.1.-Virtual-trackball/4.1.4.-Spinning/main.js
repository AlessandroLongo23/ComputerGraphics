...
const mapMousePosition = (event) => {
    ...
    let mouseVirtualX = map(event.clientX, 0, canvas.width, 1, -1);
    let mouseVirtualY = map(event.clientY, 0, canvas.height, -1, 1);

    d = Math.sqrt(mouseVirtualX * mouseVirtualX + mouseVirtualY * mouseVirtualY);
    if (d <= 1 / Math.sqrt(2))
